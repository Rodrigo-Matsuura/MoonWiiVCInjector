...
internal class NkitReaderGc : IReader
{
    ...
    public void Read(Context ctx, NStream inStream, Stream outStream, Coordinator pc)
    {
        ...
        try
        {
            ...
            NStream target = new(new CryptoStream(outStream, crc, CryptoStreamMode.Write));
            ...
            List<ConvertFile> conFiles = NkitFormat.GetConvertFstFiles(inStream.Length, hdr, fst, true, -1, out string error);

            if (conFiles == null)
            {
                if (error != null)
                    _log?.LogDetail(error);
                ConvertFile cf = new(inStream.Length - srcPos, true)
                {
    ...
    private static long CopyFile(ConvertFile file, ref long nullsPos, ref long srcPos, long dstPos, long imageSize, Stream inStream, Stream target)
    {
    ...
    private static long WriteGap(ConvertFile file, ref long nullsPos, ref long srcPos, long dstPos, NStream inStream, Stream target, bool firstOrLastFile)
    {
        if (file.GapLength == 0)
        {
        ...
        else
        {
            ...
            while (prg > 0)
            {
                ...
                else if (bt == GapBlockType.ByteFill)
                {
                    ...
                    bytes = Math.Min(cnt * Gap.BlockSize, prg);
                    Stream bs = btByte switch
                    {
                        0x00 => ByteStream.Zeros,
                        0x55 => ByteStream.Fives,
                        0xff => ByteStream.FFs,
                        _ => new ByteStream(btByte)
                    };
