...
internal class NkitReaderWii : IReader
{
    ...
    public void Read(Context ctx, NStream inStream, Stream outStream, Coordinator pc)
    {
        try
        {
            ...
            long discOffset = 0;
            List<NkitPartitionPatchInfo> patchInfos = [];
            discOffset += hdr.Size;
            ...
            ScrubManager scrubFiller = new(null);
            bool isRecoverable = false;

            using (NDisc disc = new(_log, inStream))
            {
                ...
                CryptoStream crcStream = new(outStream, crc, CryptoStreamMode.Write); //wrap to calculate crc
                ...
                foreach (WiiPartitionInfo part in hdr.Partitions) //already sorted
                {
                    if (updatePartitionCrc != 0 && srcPos == hdr.Size) //write update partition out
                    {
                        ...
                        {
                            isRecoverable = true;
                            string msg = string.Format("Update Partition is not present. Recovery files are required to convert to ISO: {0}", updateFileName);
                            _log?.LogDetail(msg);
                    ...
                    NkitPartitionPatchInfo patchInfo = new() { HashGroups = [] };
                    ...
                    LongRef origSize = new() { Value = 0 };
                    WiiPartitionGroupSection wp = null;
                    WiiPartitionHeaderSection wh = new(hdr, null, part.DiscOffset, patchInfo.PartitionHeader.Data, patchInfo.PartitionHeader.Data.Length);
                    MemorySection ph = new(new byte[0x8000 * 64]);
                    long remaining = long.MaxValue; //set after first block read
                    int groupIndex = 0;
                    WiiHashStore hashes = new();
                    ...
                    try
                    {
                        using (prtStream = new StreamCircularBuffer(0, null, null, output => srcPos += PartitionStreamWrite(origSize, inStream, output, size, patchInfo, hashes, pc)))
                        {
                            int gs = 0;
                            int ge = 0;
                            int i = 0;
                            ...
                            while (remaining > 0)
                            {
                                ...
                                RepairBlocks(wp, scrubbed, blocks); //only test if the hashes aren't preserved (only preserved for scrubbed/customs)
                ...
                {
                    JunkStream partJunk = new(lastPartitionType != PartitionType.Data ? hdr.ReadString(0, 4) : lastPartitionId, hdr.Read8(6), lastPartitionType == PartitionType.Update ? 0 : imageSize);
    ...
    private static bool RepairBlocks(WiiPartitionGroupSection wp, int scrubbed, int blocks)
    {
        bool isValid = true;
    ...
    private static void PatchGroups(NkitPartitionPatchInfo patchInfo, WiiDiscHeaderSection discHeader, long crcPatchOffset, byte[] crcPatchData)
    {
        ...
        WiiPartitionHeaderSection wh = new(discHeader, null, partDataOffset, patchInfo.PartitionHeader.Data, patchInfo.PartitionHeader.Size)
        {
            ScrubManager = patchInfo.ScrubManager
        };

        wh.Initialise(true, patchInfo.PartitionDataHeader.ReadString(0, 4));
        WiiPartitionGroupSection wp = new(discHeader, wh, data, partDataOffset, Math.Min(WiiPartitionSection.GroupSize, crcPatchData.Length), true);

        if (patchInfo.Fst != null)
        {
            using MemoryStream fstStream = new(patchInfo.Fst.Data);
            FstPatch(patchInfo, wp, crcPatchOffset, crcPatchData, fstStream);
        }

        MemorySection d = new(crcPatchData);
    ...
    private static void HashPatchGroup(NkitPartitionPatchInfo pi, WiiPartitionGroupSection wp)
    {
    ...
    private static void FstPatch(NkitPartitionPatchInfo pi, WiiPartitionGroupSection wp, long patchOffset, byte[] crcPatchData, Stream fst)
    {
        ...
        using MemoryStream dest = new(crcPatchData);
        ...
        for (int i = gs; i <= ge; i++)
        {
            ...
            RepairBlocks(wp, scrubbedCount, bnkCount);
    ...
    private long PartitionStreamWrite(LongRef outSize, Stream inStream, Stream target, long size, NkitPartitionPatchInfo patchInfo, WiiHashStore hashes, Coordinator pc)
    {
        ...
        try
        {
            if (hdr.ReadString(0, 4) == "\0\0\0\0")
            {
                ...
                JunkStream junk = new(hdr.Read(0, 4), hdr.Read8(6), outSize.Value); //SET LENGTH FROM HEADER
            ...
            else
            {
                ...
                JunkStream junk = new(hdr.Read(0, 4), hdr.Read8(6), imageSize); //SET LENGTH FROM HEADER
                ...
                List<ConvertFile> conFiles = NkitFormat.GetConvertFstFiles(size, hdr, fst, false, -1, out string error);

                if (conFiles == null)
                {
                    if (error != null)
                        _log?.LogDetail(error);
                    ConvertFile cf = new(imageSize - srcPos, true)
                    {
                        FstFile = new(null) { DataOffset = hdr.ReadUInt32B(0x424), Offset = hdr.ReadUInt32B(0x424), Length = (int)fst.Size },
                    };
    ...
    private static long CopyFile(ConvertFile file, ref long nullsPos, ref long srcPos, long dstPos, Stream inStream, Stream target)
    {
    ...
    private static long WriteGap(ConvertFile file, ref long nullsPos, ref long srcPos, long dstPos, Stream inStream, Stream target, JunkStream junk, bool firstOrLastFile, ScrubManager scrub)
    {
        long fileLength = file.FstFile.Length;
        LongRef gapLength = new() { Value = file.GapLength };
    ...
    private static long WriteFiller(ref long srcPos, long dstPos, long nullsPos, Stream inStream, Stream target, JunkStream junk, ScrubManager scrub)
    {
    ...
    private static long WriteGap(ref long fileLength, LongRef gapLength, ref long nullsPos, ref long srcPos, long dstPos, Stream inStream, Stream target, JunkStream junk, bool firstOrLastFile, ScrubManager scrub)
    {
